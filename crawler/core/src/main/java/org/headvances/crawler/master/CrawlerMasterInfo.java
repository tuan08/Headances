...
public class CrawlerMasterInfo implements Writable {
  ...
  public CrawlerMasterInfo(boolean loadJVMInfo) {
  	if(loadJVMInfo) {
			jvmInfo.put(new Text("jvm.memory"), new Text(JVMInfoUtil.getMemoryUsageInfo())) ;
			jvmInfo.put(new Text("jvm.gc"), new Text(JVMInfoUtil.getGarbageCollectorInfo())) ;
			jvmInfo.put(new Text("jvm.thread"), new Text(JVMInfoUtil.getThreadInfo(true))) ;
		}
  }
