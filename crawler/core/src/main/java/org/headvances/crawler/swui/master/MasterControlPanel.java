package org.headvances.crawler.swui.master;
...
import org.headvances.crawler.swui.comp.DataProcessInfoPanel;
import org.headvances.crawler.swui.comp.QueueNode;
import org.headvances.crawler.swui.comp.TreeNodeListener;
import org.headvances.crawler.swui.comp.URLDatumCommitInfoPanel;
import org.headvances.crawler.swui.comp.URLDatumScheduleInfoPanel;
...
public class MasterControlPanel extends JPanel {
  ...
  public void onConnect(ClusterClient client) {
  	removeAll() ;
  	Set<Member> set = client.getMembers("master") ;
  	...
  	MouseListener ml = new MouseAdapter() {
  		public void mousePressed(MouseEvent e) {
  			if(e.getClickCount() > 1) {
            	...
            	SwingUtil.showError(MasterControlPanel.this, "Component Info Error", ex) ;
            }
  				}
  			}
  		}
  ...
  private void add(DefaultMutableTreeNode memberNode, Member member) {
  	...
  	qcat.add(new QueueNode(member, "Fetch Data", "FetchDataQueueScheduler", "JMSChannelListener")) ;
  	qcat.add(new QueueNode(member, "Commit URL Fetch", "URLDatumCommitFetchQueueScheduler", "URLDatumCommitFetchQueueListener")) ;
  	qcat.add(new QueueNode(member, "URL Fetch Log", "URLDatumFetchLogQueueScheduler", "URLDatumFetchLogQueueListener")) ;
  	...
  	memberNode.add(new ComponentNode("SiteURLDatumInfo", member, "URLDatumFetchScheduler", "") {
  		public void onSelect() throws Exception {
  			CrawlerPane pane = CrawlerApplicationPlugin.getInstance().getCrawlerPane() ;
  			WorkingWorkspace wspace = pane.getWorkingWorkspace() ;
  			wspace.addTabView("URLDatum Schedule Info", new URLDatumScheduleInfoPanel(this)) ;
  		}
  	}) ;
  	memberNode.add(new ComponentNode("URLDatumCommitInfo", member, "URLDatumFetchScheduler", "") {
  		public void onSelect() throws Exception {
  			CrawlerPane pane = CrawlerApplicationPlugin.getInstance().getCrawlerPane() ;
  			WorkingWorkspace wspace = pane.getWorkingWorkspace() ;
  			wspace.addTabView("URLDatum Commit Info", new URLDatumCommitInfoPanel(this)) ;
  		}
  	}) ;
  	
  	memberNode.add(new ComponentNode(member, "FetchDataProcessor", "") {
  		public void onSelect() throws Exception {
  			CrawlerPane pane = CrawlerApplicationPlugin.getInstance().getCrawlerPane() ;
  			WorkingWorkspace wspace = pane.getWorkingWorkspace() ;
  			wspace.addTabView("Fetch Data Processor", new DataProcessInfoPanel(this)) ;
  		}
  	}) ;
