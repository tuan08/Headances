...
public class URLVector {
  ...
  public URLVector(String url) {
    this.url = url;
    try {
      urlnormalizer = new URLNormalizer(URLDecoder.decode(url.toLowerCase(), "UTF-8"));
    } catch (Exception e) {
  ...
  public String[] getVector() {
    ...
    URLNormalizer normalizer = getURLNormalizer();
    ...
    for (int i = 0; i < segs.size(); i++) {
      ...
      if (i == segs.size() - 1) {
        if (segs.get(i).contains(".")) {
          features.add("ext:" + seg.substring(seg.lastIndexOf(".") + 1)
              + ":deep:" + (i + 1));
