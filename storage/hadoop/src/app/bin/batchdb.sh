...
if [ "$COMMAND" = "export" ] ; then
  CLASS='org.headvances.hadoop.batchdb.document.Export'
  ...
  CLASS='org.headvances.hadoop.batchdb.document.Import'
  ...
  CLASS='org.headvances.hadoop.batchdb.document.DocumentDBStat'
  JAVA_OPTS="-server -XX:+UseParallelGC -Xshare:auto -Xms128m -Xmx512m"
  if [ "$MODE" = "-deamon" ] ; then
    nohup $JAVACMD $JAVA_OPTS $HADOOP_OPTS -cp $CLASSPATH $YOURKIT_PROFILE_OPTION $SYS_PROPS $CLASS "$@" > $LOG_FILE 2>&1 < /dev/null &
    printf '%d' $! > $PID_FILE
